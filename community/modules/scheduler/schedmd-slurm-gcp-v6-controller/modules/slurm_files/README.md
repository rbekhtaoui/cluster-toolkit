...
| <a name="input_slurm_qos_list"></a> [slurm\_qos\_list](#input\_slurm\_qos\_list) | Slurm QoS definitions with specs and target assignments (partitions, accounts, users). | <pre>map(object({<br/>    specs = map(string),<br/>    targets = list(object({<br/>      type    = string,<br/>      names   = list(string),<br/>      default = bool<br/>    })),<br/>  }))</pre> | n/a | yes |
